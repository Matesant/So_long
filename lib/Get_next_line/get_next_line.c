...
/*   Updated: 2023/11/08 16:33:27 by matesant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_get_new_line(int fd, char *new_line)
{
	...
	if (!line_buffer)
		return (NULL);
	if (!new_line)
		new_line = gnl_ft_strdup("");
	read_bytes = 1;
	while ((!gnl_ft_strchr(new_line, '\n')) && read_bytes)
	{
		...
		new_line = gnl_ft_strjoin(new_line, line_buffer);
	}
	free(line_buffer);
	return (new_line);
}
...
static char	*ft_make_new_line(char *content_line)
{
	...
	line_position = gnl_ft_strchr(content_line, '\n');
	if (line_position == NULL)
		return (gnl_ft_strdup(content_line));
	else
		line_size = ((line_position - content_line) + 2);
	new_line = malloc(line_size);
	gnl_ft_strlcpy(new_line, content_line, line_size);
	return (new_line);
}
...
static char	*ft_update_after_new_line(char *line_to_update)
{
	...
	line_position = gnl_ft_strchr(line_to_update, '\n');
	if (line_position == NULL)
	{
		free(line_to_update);
		return (NULL);
	}
	rest_of_the_line = gnl_ft_strdup(line_position + 1);
	free(line_to_update);
